...
function EditNote() {
  ...
  useEffect(() => {
    axios
      .get(`http://localhost:3000/api/notes/${location.state.id}`)
      .then((response) => {
        console.log(response.data.NotebyId);
        setTitle(response.data.NotebyId.title);
        setContent(response.data.NotebyId.content);
      })
      .catch((error) => {
        console.error(error);
      });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setMessage("Error Editing note. Try again.");
    }
  };
